...
monitor_metrics() {
  ...
  # /home/arthurpassos/perf-build/WSL2-Linux-Kernel/tools/perf/perf stat -p $pid -o "$results_dir/${prefix}_perf_$iteration.log" &
...
run_openmp_program() {
  ...
  for iteration in $iterations; do
    ...
    # Run the OpenMP program with perf record
    # /home/arthurpassos/perf-build/WSL2-Linux-Kernel/tools/perf/perf record -o per-out.txt -p 753221
    /home/arthurpassos/perf-build/WSL2-Linux-Kernel/tools/perf/perf record -o "$results_dir/openmp_perf_$iteration.data" "../programs-original/${program%.c}" >/dev/null 2>&1
    /home/arthurpassos/perf-build/WSL2-Linux-Kernel/tools/perf/perf report -i "$results_dir/openmp_perf_$iteration.data" > "$results_dir/openmp_perf_$iteration.txt"
